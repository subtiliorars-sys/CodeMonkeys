/* CodeMonkeys web terminal — Claude Code-style REPL fallback over the existing
   session/agent loop (docs/TERMINAL_DESIGN.md). Vanilla JS, no deps, no CDN.
   SECURITY (design fix F4): every piece of server/model text is rendered via
   textContent only — never innerHTML — and ANSI escapes are stripped. */
"use strict";
...
function renderEvent(e) {
  switch (e.type) {
    ...
    case "agent_start": line(`  🐒 deployed ${e.agent} [${e.tier} · ${e.model}] — ${e.task}`, "t-agent"); break;
    case "agent_end": line(`  🐒 ${e.agent} reported back`, "t-agent"); break;
    case "cost": line(`  $ ${e.model} · ${e.in_tokens}→${e.out_tokens} tok · $${Number(e.usd).toFixed(4)}`, "t-dim"); break;
    case "approval":
      state.pendingApproval = e.approval_id;
      line(`⚠ APPROVAL REQUIRED: ${e.command}`, "t-warn");
      line(`  type /approve to allow or /deny to refuse`, "t-warn"); break;
    case "approval_result":
      state.pendingApproval = null;
      line(e.approved ? "  ✓ approved" : "  ✗ denied", "t-dim"); break;
    case "debate_verify":
...
async function slash(text) {
  ...
  switch (cmd) {
    ...
    case "new":
      ...
      if (arg) { const d = await api("/api/sessions", "POST",
        { title: arg, repo: "", ...(state.nextBudget ? { budget_usd: state.nextBudget } : {}) });
        state.sid = d.id; state.nextBudget = null;
        line(`(new session ${d.id} · budget $${d.budget_usd})`, "t-dim"); startPolling();
      } else { line("(next message starts a fresh session)", "t-dim"); }
      break;
    case "use":
    ...
    case "mode":
      ...
      state.mode = arg; line(`(mode → ${arg})`, "t-dim"); break;
...
input.addEventListener("keydown", async (ev) => {
  ...
  catch (err) { line(`✗ ${err.message}`, "t-err"); }
});
